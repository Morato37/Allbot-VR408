 ...
 * Allbot VR408
 *
 * With this code the Allbot VR408 will perform a random 
 * sequence of diffrent movements
 *
 */
...
{ 
  ...
  BOT.attach(hipFrontLeft,   11,  120, 1, 0);
  BOT.attach(hipFrontRight,  2,  120, 0, 0);
  BOT.attach(hipRearLeft,     9,  120, 0, 0);
  BOT.attach(hipRearRight,    4,  120, 0, 0);
 
  BOT.attach(kneeFrontLeft,  A2,  10, 1, 0); 
  BOT.attach(kneeFrontRight,  A3,  10, 0, 0);
  BOT.attach(kneeRearLeft,   10,  10, 1, 0);
  BOT.attach(kneeRearRight,   3,  10, 0, 0);
  ...
  if (IRreceive == false)                  // Check if required (when Serial is started servo1 connector will not work!)
    {
      Serial.begin(2400);
